...
class DrawView(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    var watchDrawListener: WatchDrawListener? = null
    ...
    private val bitmapLeft: Int
        get() = (width - (bitmap?.width?:0)) / 2

    // Centered vertically
    private val bitmapTop: Int
        get() = (height - (bitmap?.height?:0)) / 2
    ...
    fun setPoints(points: List<PointF>) {
        ...
        for (i in 1 until points.size - 1) {
            ...
            currentPath.quadTo(points[i].x, points[i].y, midX, midY)
        }
        currentPath.lineTo(points.last().x, points.last().y)
